...
export class AreaSelectScene extends Phaser.Scene {
  ...
  private render(): void {
    ...
    this.add.text(195, 112, this.message, { fontSize: "18px", color: COLORS.text, fontStyle: "bold" }).setOrigin(0.5);

    AREAS.forEach((area, index) => {
      const unlocked = state.unlockedAreas.includes(area.id);
      const y = 178 + index * 112;
      addPanel(this, 195, y, 326, 92);
      this.add.text(102, y - 24, area.name, { fontSize: "18px", color: COLORS.text, fontStyle: "bold" }).setOrigin(0, 0.5);
      this.add.text(102, y + 4, `ST ${area.staminaCost} / 深度 ${area.maxDepth}m`, {
        fontSize: "13px",
        color: COLORS.muted
      });
      this.add.text(102, y + 28, "★".repeat(area.difficulty) + "☆".repeat(5 - area.difficulty), {
        fontSize: "16px",
        color: "#f5b83f"
      });
      addButton(
        ...
        92,
        42,
        unlocked ? `推奨\nLv.${area.recommendedLevel}` : "解放",
        () => this.selectArea(area.id, unlocked),
        unlocked ? COLORS.green : 0x555555
      );
    });

    addButton(this, 82, 790, 120, 52, "戻る", () => this.scene.start("HomeScene"));
  }
