...
export class TitleScene extends Phaser.Scene {
  ...
  create(): void {
    ...
    addButton(this, 96, 794, 150, 48, "チュートリアル", () => this.showTutorial(), COLORS.panel);
    this.add.text(320, 790, "v0.4.0", { fontSize: "16px", color: "#d7a15b" }).setOrigin(0.5);
  ...
  private showTutorial(): void {
    const state = getGameState();
    setGameState({ ...state, settings: { ...state.settings, tutorialSeen: true } });
    const overlay = this.add.rectangle(195, 422, 390, 844, 0x000000, 0.62);
    const panel = this.add.rectangle(195, 420, 315, 300, COLORS.panel, 0.98).setStrokeStyle(2, COLORS.goldDark);
    this.add.text(195, 326, "遊び方", { fontSize: "24px", color: COLORS.text, fontStyle: "bold" }).setOrigin(0.5);
    this.add
      .text(195, 420, "数字は周囲8マスの地雷数です。\n安全な壁を掘って進み、地雷候補は長押しでマーク。\n地雷は冷却または解除してから壊すと素材になります。", {
        fontSize: "15px",
        color: COLORS.text,
        align: "center",
        lineSpacing: 8,
      ...
      .setOrigin(0.5);
    addButton(this, 195, 548, 180, 52, "OK", () => {
      overlay.destroy();
      panel.destroy();
      this.scene.restart();
    }, COLORS.green);
