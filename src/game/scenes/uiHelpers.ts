...
export function addSmallLabel(scene: Phaser.Scene, x: number, y: number, text: string): Phaser.GameObjects.Text {
  return scene.add
    .text(x, y, text, {
      fontFamily: "sans-serif",
      fontSize: "14px",
...
export function addHudBar(
  ...
  const bg = scene.add.rectangle(0, 0, width, 18, 0x130d08).setStrokeStyle(1, 0x6b4725);
  const fillWidth = Math.max(0, Math.round((width - 4) * (value / max)));
  const fill = scene.add.rectangle(-width / 2 + 2, 0, fillWidth, 12, color).setOrigin(0, 0.5);
  const text = scene.add
...
export function drawPixelMiner(scene: Phaser.Scene, x: number, y: number): Phaser.GameObjects.Container {
  const shadow = scene.add.ellipse(0, 13, 24, 8, 0x000000, 0.35);
  const body = scene.add.rectangle(0, 4, 16, 20, 0x8b5a2b).setStrokeStyle(1, 0x33200f);
  const head = scene.add.circle(0, -8, 8, 0xd69a57).setStrokeStyle(1, 0x33200f);
  const helmet = scene.add.rectangle(0, -14, 20, 8, 0xe1a82c).setStrokeStyle(1, 0x593711);
