...
export function craftRecipe(
  ...
  if (
    ...
    return {
      crafted: false,
      message: `${getEquipment(recipe.output.equipmentId).name}は所持済みです`,
      state,
    };
  }
      ...
      state: {
        ...
        collection: {
          ...
          equipment: Array.from(
            new Set([...paidState.collection.equipment, equipmentId]),
          ),
        },
      },
    };
...
export function craftFirstAvailable(state: SaveData): CraftResult {
  const recipe = RECIPES.find(
    (candidate) =>
      state.player.coins >= candidate.cost.coins &&
      hasItems(state.inventory, candidate.cost.items),
  );
  if (!recipe) {
